...
<Text>Email</Text>
<Link
  href="mailto:pauliescanlon@gmail.com"
  target="_blank"
  variant="styles.a"
>
  ...
  {sourceMonths => {
    ...
    return (
      <Box sx={{ backgroundColor: 'surface', p: 3 }}>
        <Heading variant="styles.h4" sx={{ color: "primary" }}>{currentYear[0].year}</Heading>
...
<SourceMonths>
  {sourceMonths => {
    ...
    return (
      <Box sx={{ backgroundColor: 'surface', p: 3 }}>
        <Heading variant="styles.h4" sx={{ color: "secondary" }}>{currentYear[0].year}</Heading>
        <Box sx={{ display: 'flex', flex: '1 1 auto', height: 181 }}>
          <Flex sx={{ flexWrap: 'wrap', width: '100%' }}>
            {currentYear.map((month, index) => {
              const { initial, count, percent } = month
