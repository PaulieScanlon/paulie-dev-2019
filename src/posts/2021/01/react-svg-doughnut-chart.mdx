...
This post is largely based on this [article](https://medium.com/@heyoka/scratch-made-svg-donut-pie-charts-in-html5-2c587e935d72) by [Mark Caron](https://twitter.com/heyoka) and using this as the foundation i'm going to explain how I created the "tags" chart seen in my [dashboard](/dashboard)
...
In the above snippet I use `array.reduce()` in order to determine if it's an `existingItem` where i'll increment the `count` or return a new item with a `count` of 1 . Using `array.find()` I loop over the `items` to see if the array contains the `item` passed from the params in the _parent_ `array.reduce()` 😓
...
The `r` value is where things get a bit _mathematical_. `r` is the [radius](https://developer.mozilla.org/en-US/docs/Web/SVG/Attribute/r) of the circle.
