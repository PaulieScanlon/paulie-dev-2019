...
The component we're going to build looks like the below and i've called it `<MrPragma />`. You can find the `src` [here](https://github.com/PaulieScanlon/paulie-dev-2019/tree/master/src/posts/2021/01/MrPragma.js)
...
The theme object is a little bit like a global CSS file but without all the headaches.

This is key to Theme UI so bookmark the [theme-spec](https://theme-ui.com/theme-spec) page as i'll be referring it frequently.

When your first start your site after installing Theme UI you'll notice the background is a kind of purple color. That's fine, it means Theme UI is working and is applying the default styles.
...
This is my opinion is one of the hardest concepts to grasp. The TLDR is Theme UI has made decisions regarding what CSS properties map to what keys in the theme object. The HTML `color` and `background-color` are mapped to the `colors` key which is why the changes above will have an effect on your overall site theme. More on this later
...
The reason you don't need to write `theme.colors.text` is because Theme UI shorthands the OOP lookup for us by mapping CSS properties of certain types to a constrained set of keys from the theme object
