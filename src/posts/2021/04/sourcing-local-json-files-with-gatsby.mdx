...
The first two methods will also use
...
method to create the "post" pages seen in the demo
...
1. Fist I `require` the single `.json` file called `MOCK_DATA.json`.
2. Then by using the `sourceNodes` extension point so I can access
   [Gatsby's Actions](https://www.gatsbyjs.com/docs/reference/config-files/actions/) plus add my own code.
3. I've use `forEach` to iterate over every object contained within the
...
The query names E.g `post` and `allPost` are created by Gatsby from the name provided by `internal.type`
