...
_The above is actually default Gatsby behavior and not related to the Files System Route API but I did want to point out
that Gatsby is able to create routes from any `.js|tsx|md|mdx` file(s) found under `src/pages`_
...
chose to locate your source files and those curly braces are a bit odd when you first seem them.
