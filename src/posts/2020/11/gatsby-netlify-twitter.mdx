...
Apologies in advance for the rather long-winded blog title but as it suggests in this post i'm going to explain how you can use [Netlify Functions](https://www.netlify.com/products/functions/) to access your Twitter profile data using the [Twitter v2 API](https://developer.twitter.com/en/docs/twitter-api/early-access) and display it on your Gatsby blog.

## A rather unique requirement

This might be a specific to me but I wanted to solve a little problem I was having with my "digital footprint". As you can see I have this blog: [https://paulie.dev](https://paulie.dev/) and a commercial portfolio: [https://www.pauliescanlon.io](https://www.pauliescanlon.io/)

Both sites are built on top of my Gatsby theme: [gatsby-theme-terminal](https://gatsby-theme-terminal.netlify.app/) which is Open source and can be found on my [GitHub](https://github.com/PaulieScanlon/gatsby-theme-terminal)
...
One option I considered would have been to hook up Content Management System, and this would have been fine and it would have kept both my sites in sync but it wouldn't have been able to update my Twitter profile blurb...
...
- GitHub repo [https://github.com/PaulieScanlon/gatsby-netlify-twitter](https://github.com/PaulieScanlon/gatsby-netlify-twitter)

... but the actual API I use for my blog and site is here: [https://paulie-api.netlify.app](https://paulie-api.netlify.app/)

## Tech

### Netlify Functions

"Power your site without managing servers" is how Netlify describe Functions and all for intents and purposes thats exactly what they are. Similar to how you might create an [Express](https://expressjs.com/) app and deploy it somewhere but without the hassle of having to setup server side environments and more crucially any really dweeby server uptime monitoring.
...
Apply for access to the [Twitter API](https://developer.twitter.com/en/products/twitter-api). This is quite a lengthy process so strap in and also bookmark this post as it might take a few days for Twitter to accept your application.

Once you have access you can head over to the [Developer Portal](https://developer.twitter.com/en/portal/dashboard) and create a new project, and within the project you can create an "app", I called mine "paulie-api".
...
To run Netlify Functions we'll be using `netlify dev` rather than `gatsby develop` or `yarn develop` so you'll need to install the [Netlify CLI](https://docs.netlify.com/cli/get-started/)
...
`functions` is kind of it's own application so it'll need it's own `package.json` and will have one dependency on [twitter-v2](https://github.com/HunterLarco/twitter-v2)
...
Next have a look at [.env.example](https://github.com/PaulieScanlon/gatsby-netlify-twitter/blob/main/.env.example). You'll need to create your own `.env` file and add the environment variables as seen in the `.env.example`. Naturally you'll want to change the `GATSBY_TWITTER_USERNAME` to your own Twitter username and the Twitter keys and tokens will be what I referenced earlier which are provided by the Twitter Developer Portal
...
In the above you can see we use our `client` to hit the `users/by/username` Twitter API endpoint which you can read more about [here](https://developer.twitter.com/en/docs/twitter-api/users/lookup/introduction), which returns a `data` object which I pass on to the callback body as `{ user: data }`

This is the object that'll we receive in our frontend

The next bit will greatly depend on how you've set up your frontend but in the [Demo](https://github.com/PaulieScanlon/gatsby-netlify-twitter/blob/main/src/pages/index.js) I have one `page` called `index.js` which uses a `useEffect` to "fetch" the data from the Netlify Function.
...
You might be wondering why this environment variable is prefixed with `GATSBY_`. This is so Gatsby can access it from the frontend. You can read more about Gatsby environment variables [here](https://www.gatsbyjs.com/docs/environment-variables/#client-side-javascript)
...
And to ensure when we deploy everything works as it should you'll need to modify your [`netlify.toml`](https://github.com/PaulieScanlon/gatsby-netlify-twitter/blob/main/netlify.toml)

For the most part `netlify dev` will attempt to automatically determine which static site generator you're using by looking for certain files in the project root. If you don't already have a `gatsby-config.js` at the root of your project add one as i've done [here](https://github.com/PaulieScanlon/gatsby-netlify-twitter/blob/main/gatsby-config.js)... you might not want to use Theme UI so ignore that if you're using some other CSS method.
...
With all that set you should be able to spin up `netlify dev` visit `http://localhost:8888/` and see your own Twitter data rendered on the page, and once you deploy you "should" find everything continues to work as it did locally.
...
Fingers crossed this is enough to get you up and running with Netlify Functions, but if you feel like any of this is a bit "[draw the rest of the fucking owl](https://www.reddit.com/r/restofthefuckingowl/)" feel free to find me on Twitter.
