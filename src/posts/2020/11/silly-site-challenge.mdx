...
The Bum Search section features a fully functioning Search input, with a Select to change which value from the data you want to filter on, along with table colum sorting functionality so users can explore the full data set returned by GitHub.
...
This latest update pretty much completes my "MVP" but i've had another idea, it's again grounded in my desire to demonstrate Gatsby is React and can do _all the things_ that Create React App can do so stay tuned and i'll be back with another update in a week or so.
