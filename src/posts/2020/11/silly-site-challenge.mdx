...
Using a config object like this is great for the Bum Bum Maker UI elements but it's a bit verbose to drive the Svg conditional `<g>` rendering.... allow me to explain.

By using my old chum [Array.prototype.reduce](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/reduce) I can "loop" over the items in the `bumBumConfig` array and pick out just the ones that have `checked: true`
...
This results in an array of strings for each _part_ that is checked eg
...
Then I can use these value to _pick_ a key from a [properties](https://github.com/PaulieScanlon/bumhub/blob/main/src/components/maker-bum/maker-bum.tsx#L40) dispatch table... again i've removed a lot from the below example for brevity.
...
For values in the results array that are present in the `properties` `config` object I return the Svg `<g>` element(s) and for values that aren't I return `null`. This allows me to conditionally render all Svg `<g>` elements that together form the Svg you see in browser.

If you open up your developer tools and find the Svg element, then click around on the Radio buttons you'll see the Svg re-renders it's child `<g>` elements according to the checked values provided by the Radio buttons. I typically use this approach in my day job to build complex web applications for companies that make bazillions that no one cares about but here i'm using the same approach to conditional render an Svg Bum graphic which really puts the _fun_ into Functional Programming.
...
My next "sprint" if you like will focus on accessibility and performance. Having reviewed the [Pre-submission checklist](https://www.gatsbyjs.com/silly-site-challenge-faq/#pre-submission-checklist) I do indeed want to make Bum Hub as good and silly as it can be.
