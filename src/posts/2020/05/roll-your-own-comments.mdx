...
- `GATSBY_SHOW_SIGN_UP` is used to hide the Sign up button when the site is in production and when you’re
  running `yarn serve` locally.
...
We need to create a few more comments to while we're in development so change the hard-coded values for `name` and `comment` and run the following again.
...
Now that we can create comments it's also required that we can delete a comment.

By adding a `commentId` of one of the comments you created above you can delete it from the database.

We're not really concerned with the return value here but if you wanted to surface this in the UI you could use the `commentId` return value
...
The shape of the returned result here is an array of the same shpare we defined in the _Index_ values
