...
This tutorial will only focus on the operations we need to use [FaunaDB](https://fauna.com/) to power a comment system for a Gatsby blog. The finished article can be deployed to Netlify and takes advantage of Netlify Identity and Netlify's serverless functions. Just two things that make the JAM stack really powerful.

You can see the finished application [here](https://fauna-gatsby-comments.netlify.app/)

The repo below is a fully built Gatsby application and the steps to build this are _not_ covered in this tutorial but we will be using it to demo how to use FaunaDB.
...
- `GATSBY_FAUNA_DB` is the FaunaDB secret key for our database

<Divider />

If you’re curious you can run `yarn develop` and then navigate to http://localhost:8000 in your browser.

Now we’re going to write some of the operations we need to use FaunaDB
...
Next we need to create an **INDEX** in FaunaDB, an **INDEX** allows us to query the database with a specific term
...
When we query the database using this index we can tell FaunaDB which parts of the entry we want to return.

You can do this by adding values but be careful to enter the values exactly as they appear above because (on the FaunaDB free tier) you can’t amend these after you’ve created them so if there’s a mistake you’ll have to delete the index and start again.
...
We're going to take a similar approach as above but this time create a new index that allows us to query FaunaDB in a different way.
