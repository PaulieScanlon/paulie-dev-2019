...
This tutorial will only focus on the operations we need to use [FaunaDB](https://fauna.com/) to power a comment system for a Gatsby blog. The finished app can be deployed to Netlify and takes advantage of Netlify Identity and Netlify's serverless functions. Just two things that make the JAM stack really powerful.

You can see the finished comments app [here](https://fauna-gatsby-comments.netlify.app/)

The repo below is a fully built Gatsby app and the steps to build this are _not_ covered in this tutorial but we will be using it to demo how to use FaunaDB.
...
For now we’re going to hard-code `slug`, `name` and `comment` but in the final app these values are captured by the input form on the posts page and passed in via args
...
Naturally only approved comments will be displayed on the post and deleted ones are gone forever.
