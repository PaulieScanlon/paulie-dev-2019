...
```javascript
// some-file.js
import myIcon from "./icons/icon-a.svg"
import myIcon from "./icons/icon-b.svg"
import myIcon from "./icons/icon-c.svg"
import myIcon from "./icons/icon-d.svg"
```
...
As you're probably aware "design systems" are a current trend and more or less they attempt to reduce the amount of "one off" styles in any code base in favour of a more general re-usable set of styles that work in a number of places around the code base. This approach makes the code base easier to maintain, usually results in fewer bugs and if a design system is created and used correctly it can **dramatically** speed up development and cut down on development costs.
...
The "S" in **S**vg stands for **scalable**. This means _it doesn't matter what size the icons are designed at_ because we can use CSS and or HTML attributes to make them bigger or smaller without losing quality.

Now that's out of the way lets pick up on that point i just made...

> it doesn't matter what size the icons are designed at

Whilst this is true it **does** matter if some icons are designed at 24x24 and some are designed at 48x48.

We're going to build an icon _system_ and one of the requirements of our system is that **ALL** the icons are designed using the same specification.

The specification is as follows and _could_ be used as Acceptance Criteria if you're looking to write a ticket for your design team.
...
That last point doesn't actually matter but why design it in white and on a white artboard?

To explain a bit more i'm going to use an icon from [material icons](https://material.io/resources/icons/?style=baseline). This is `settings-24px.svg` and on the left is what it looks like in Illustrator and on the right is what it looks like in VS Code.
...
If you download any of the icons in the [material icons](https://material.io/resources/icons/?style=baseline) set you'll see the same set of rules.
...
We're now going to create a utils file to store our icon "paths" and build a React `SvgIcon` component in React
...
SvgIcon.propTypes = {
  /** The name of the key in icon-paths.js */
...
The only watch outs are getting the design team on board. If you go down this road and then later icons are provided with different viewbox sizes you can't just make a "one off" change.
