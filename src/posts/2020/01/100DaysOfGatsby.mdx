...
I've seen this on every single project i've worked on but i don't know why it always happens.
...
If B, do you find that a number of smaller components need to run their own operations to wrangle the data in to a format that is actually helpful, do you find theres a performance impact when doing this?

My preference is to look at what we're trying to build, inspect the data and either strip out or re-format before passing it on down the line... it's usually quite clear that an object key called `node_type: '___Aadhfd` probably won't be required so maybe let's get rid of it to avoid confusion. 🤷‍♂️
...
<Tweet id="PaulieScanlon/status/1234434804885655554" />
