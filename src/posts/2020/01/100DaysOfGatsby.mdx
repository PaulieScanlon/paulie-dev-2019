...
I spend the morning investigating why `gatsby-source-contentful` was throwing errors with my blog but after a few hours i took step back and thought about if i really need a CMS in my blog.

My motivation for looking into `gatsby-source-contentful` because it was one of the challenges and i suppose i do have a requirement for both my blog and my site to share the some of the same content but since this is a source plugin i'd still need to build and deploy both in order ot keep them in sync... and if i need to do that it's probably the same amount of work involved in just copy and pasting the one part of thing that they share.

I did then look in to perhaps making a REST-ful request to contentful which would remove the static part and probably have a negative effect SEO but i was just curious... i got all the way through the docs, set up a nice component that will make a request with the correct space id and bearer token and then moved on to work out what the GraphQL query would be... i then discovered that on the free plan you can't make GraphQL queries so that was the morning pretty much wasted 😂

So instead of messing around with things i can't control i've messed around with some things i can. There were a few issues with my theme [gatsby-theme-terminal](https://github.com/PaulieScanlon/gatsby-theme-terminal) which i wanted to look at.

The first relates to the `prev` and `next` links that "were" at the bottom of each "post" page but.... This theme has the ability to query from any local data source and although in "your site" you can filter and sort however you want the GraphQL query that's inside the theme is still in charge of source the nodes and deciding what in fact is `prev` or `next` if you have two sources and filter out one in your MDX the underlying nodes won't know about that and might try to link to a `prev` or `next` page/post that isn;t relevant ... so i've removed this functionality ... sorry abou that.

There was also an issue with the `<SourceDays />` component where i'm performing some calculations based on what `date` is found in the front matter `new Date(item.node.frontmatter.date).getDay()` unfortunately i'd forgotten that arrays start at `0` so all days where shifted forward by one change that line to `new Date(item.node.frontmatter.date).getDay() -1` fixes the problem.

I also added a _word count_, _time to read_ and _author_ to the post layout template. These fields were already there in frontmatter i just forget to add them to the UI.
...
I totally think this is possible if i were to write a _build time_ Gatsby plugin that once configured via `gatsby-config.js` using an id or something would connect to a (yet to be developed) skin-api and pull down the theme object and then continue to compile the app/site/blog with that theme object.
...
Here's any example usage of a `<TagList />` **data component** i'm developing.
...
Inside the `<TagList />` component is where i query GraphQL and also do a bit of functional JavaScript to return a nice object with the `name`, `count` and `percent` of each tag found in `frontmatter`
...
Yup, Yup, Yup. Today i've released an alpha version of my new Gatsby plugin [gatsby-plugin-prop-shop](https://github.com/PaulieScanlon/gatsby-plugin-prop-shop)
