...
A note on [sem-ver](https://semver.org/spec/v2.0.0.html) strictly speaking these are breaking changes but i feel i can justify not bumping the plugin to a major release version of `1.0.0` because in my mind when a library is only a `0.x.x` to start with it's still a pre-release... even though i have already released it. 🤯

I'm not sure if this is a bad thing to do but i feel i'm going to encounter a lot more of these kinds of naming issues before i'm happy to release a stable `1.0.0`.
