...
using Tailwind and Tailwind Typography, using both **MDX 1** and **MDX 2** with the help from
...
#### Branch: feat/mdx-2 » MDX 2

- ⚙️ [feat/mdx-v2](https://github.com/PaulieScanlon/gatsby-prism-react-renderer-mdx/tree/feat/mdx-2)
  - [PR (files changed)](https://github.com/PaulieScanlon/gatsby-prism-react-renderer-mdx/pull/2/files)
  - [MDX 2 WIP Docs](https://github.com/gatsbyjs/gatsby/blob/feat/mdx-v2-docs/docs/docs/how-to/routing/mdx.md)
  - [Gatsby Cloud Preview](https://gatsbyprismreactrenderermdx-featmdx2.gtsb.io/posts/post-one/)
...
I've added an extra `<span />` element to display the language inside the code block and have added a `Fragment` to
avoid the Jsx multiple elements error. _I'll cover how to I styled this later using Tailwind._
...
In order to position the language `<span />` using `absolute` I need to ensure the parent `<pre />` had a `relative`
position style applied to it. I accomplished this by adding the following to `tailwind.config.js`.
...
I also added a new `fontSize` to `tailwind.config.js`
...
The rest of the styles added to the `<span />` are available from Tailwind's default config.
...
Here's a diff of
[\{mdx.fields\_\_slug}.js](https://github.com/PaulieScanlon/gatsby-prism-react-renderer-mdx/blob/feat/mdx-2/src/pages/posts/%7Bmdx.fields__slug%7D.js)
...
The new MDX Plugin isn't generally available (GA) just yet but the team have been doing such a super job and I'm so
excited by it that I thought I'd share how I've been using it.

I don't expect there to be any further major changes in the way you use it so once it hits GA the example should still
work. If that turns out to not be the case i'll circle back to this post and update it.
