...
and learn it. For this reason I've chosen to use [pg-promise](https://github.com/vitaly-t/pg-promise).
...
like [Prisma](https://www.prisma.io/) which will allow you to interact with an SQL database from a node.js environment
...
a Serverless Function (API Routes). You only need to create the `pg-promise` database instance once. The following
singleton pattern prevents duplicates from being created when `getDB` is called using the different methods in this
sample app.
