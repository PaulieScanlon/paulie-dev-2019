...
company probably wouldn't have been able to offer this. That said, they've just been acquired by Netlify so 🤷‍♂️.
Although, I'd imagine providing US visa sponsorship to existing employees wouldn't have been at the top of anyone's to
do list and could have delayed my plans to move. And who know's if I'd have made the cut and kept my job through the
acquisition!
...
the `ip` address to find the geographical location, looks this up to get the latitude and longitude, and then POSTs them
to a CockroachDB database ready to be consumed by the app. The locations retrieved aren't actually your location, but
they are the locations of where the Serverless Function has been deployed. Or to put it another way, they are the
...
miles up the road from me in a different town. So if you wanna **anonymously** submit I can start to get a picture of
where in the world these edges are.
...
[Netlify](https://www.netlify.com/), [Gatsby Cloud](https://www.gatsbyjs.com/products/cloud/hosting) (and many, many
more) use these CDN's to deploy a website's static assets, E.g images, CSS, and JavaScript, and the closer these assets
are to you, the smaller the distance they have to travel down the phone line, and the faster a website will load.

Whilst this is great for static assets there's a problem with dynamic data. This is where Serverless Functions come in.
Serverless Functions in Jamstack web development are like special mini servers that are deployed the same way static
assets are and instead of being _always live_ and costing money, they are invoked and started-up when needed.

This is different to the traditional way "servers" work because Serverless Functions aren't running all the time on a
separate piece of hardware in a [server farm](https://dgtlinfra.com/inside-the-worlds-largest-data-center/) somewhere in
the Nevada desert, they're right there on the CDN sitting next to the static assets, however.

Serverless Functions are used to make server-side requests to other services, services like a database to retrieve data
to populate a website or application. For example, your Twitter timeline is made up of data that is stored in a
database, not static assets distributed globally on a CDN. Databases, typically can't distribute data to hundreds or
thousands of geographical locations because databases act like a single source of truth and duplicating this data across
multiple geographical locations to be nearer to you is a very difficult / nearly impossible thing to do.
...
The Edge app is as attempt to map where in the world these edge locations are and, at a later date I plan to
cross-reference these locations with where in the world you could deploy a CockroachDB Database. Unlike other database
solutions CockroachDB is "cloud native" which means it can be deployed to multiple cloud providers. For example, with
CockroachDB you could run your database on both Amazon Web Services (AWS) and Google Cloud Platform (GCP). This can
result in the faster transfer of data to users. You can see on the following links where the AWS and GCP regions are,
and with CockroachDB, architects can configure their services to meet customers where they are.
...
alike. This benefit scales up to the giant companies like [Twitter](https://twitter.com/) and
...
is solving this problem too! And speed vs consistency are somewhat at odds with one another — but that's an explanation
...
As you can see, explaining this using only the written word is quite a difficult thing to do, and its my hope that by
using a **"show, don't tell"** approach to storytelling we'll be better able to explain the complexities and business
benefits to the technical, and the not-so technical in a more engaging, informative or imaginative way.
