...
the cluster was running in. As you can see from the above, `eu-north-1` isn't an overly attractive thing to show in a
UI.
...
response.regions.map((region, index) => {
  return <span key={index}>{getInfo(region, response.cloud_provider).flag}</span>
}))
...
I'm not how many folks out there will ever need this, I did, so I made it. 🕺
